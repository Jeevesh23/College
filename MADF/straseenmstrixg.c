...
void copy(int a[N][N], int b[N][N], int r1, int r2, int c1, int c2)
{
    int i,j,x,y;
	int size = r2 - r1;
    for (i = r1, j = 0; i < r2 && j < size; i++, j++)
        for (x = c1, y = 0; x < c2 && y < size; x++, y++)
            b[j][y] = a[i][x];
}
...
void combine(int C11[N][N], int C12[N][N], int C21[N][N], int C22[N][N], int C[N][N], int n)
{
    ...
    for (i = 0; i < n / 2; i++)
        for (j = 0, k = n / 2; j < n / 2 && k < n; j++, k++)
            C[i][k] = C12[i][j];

    for (i = 0, k = n / 2; i < n / 2 && k < n; i++, k++)
        for (j = 0; j < n / 2; j++)
            C[k][j] = C21[i][j];

    for (i = 0, k = n / 2; i < n / 2 && k < n; i++, k++)
        for (j = 0, l = n / 2; j < n / 2 && l < n; j++, l++)
            C[k][l] = C22[i][j];
}
...
int main()
{
    ...
    while (n % 2 != 0)
    {
        printf("Enter the size (power of 2): ");
        scanf("%d", &n);
    }
