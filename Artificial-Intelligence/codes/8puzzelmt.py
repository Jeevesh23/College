...
def hill_climbing(start_state): 
    print("Heuristic Function: Number of Misplaced Tiles\n")   
    misplacedtiles(start_state)
    print("\n")
    print("Heuristic Function: Manhattan Distance\n")
    manhattan(start_state)
    print("\n")
    print("Heuristic Function: Euclidian Distance\n")
    euclidian(start_state)
    print("\n")


def misplacedtiles(start):
    current_state = start
    print("Initial State: ")
    print_state(current_state)
    ...
    while True:
        ...
        for state in next_states:
            heuristic = sum(abs(state[i][j] != goal_state[i][j]) for i in range(3) for j in range(3))
...
def manhattan(start):
    ...
    while True:
        ...
        for state in next_states:
            heuristic = sum(abs((state[i][j] // 3) - (goal_state[i][j] // 3)) + abs((state[i][j] % 3) - (goal_state[i][j] % 3)) for i in range(3) for j in range(3))
            if heuristic < min_heuristic:
                min_heuristic = heuristic
                next_state = state
